...
const TYPE_CODES = {
  ...
  FACILITY: ["AC", "GNR", "ATS", "FAN", "WFN", "CFN", "EFN", "SFN", "RFG", "MWV", "OVG", "WCL", "WDP", "WTK", "RMT", "FPN", "RPN", "KPS", "KPA", "TBL", "CHR", "PNO", "KPN"],
...
function normalizeVaultCctvRecords(input) {
  ...
  return input
    ...
    .map((row) => ({
      ...
      publicIp: toText(row.publicIp),
      port: toText(row.port),
      softwareApp: toText(row.softwareApp),
      username: toText(row.username),
      password: toText(row.password),
